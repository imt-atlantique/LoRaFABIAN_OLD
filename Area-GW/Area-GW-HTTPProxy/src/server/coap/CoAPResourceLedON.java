 ...
 */
public class CoAPResourceLedON extends CoapResource {
	
	private int counter = 0;
	
	public CoAPResourceLedON(String name) {
		super(name);
	}
	
	@Override
	public void handleGET(CoapExchange exchange) {
		exchange.respond("SEND LED ON request to LORA NET ("+(++counter)+") this resource is " + getName());
		//Do LORA
...
private void sendLoRADownstreamMessage() {
		...
		if (inet_socket_addr != null){
			...
			try {
				...
				byte[] resp = "LED ON!! JSON 802154COAP y PAMELACHU La Rompes Pebete!!!\n".getBytes();
				resp = server.lora.json.CoAPLedOn.get_json_packet();
				...
				// We send the data
				System.out.println("[CoAPResourceLedON]<- Sending "+ resp.length + " bytes to : " + inet_socket_addr );
			...
			System.out.println("[CoAPResourceLedON] ERROR there is no downstream protocol socket");
		}
